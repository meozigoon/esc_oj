...
const statusMap: Record<SubmissionStatus, { label: string; color: 'default' | 'success' | 'error' | 'warning' | 'info' }> = {
...
export default function StatusChip({ status, message }: Props) {
  ...
  return <Chip label={message || info.label} color={info.color} variant="outlined" />;
}

...
export default function ProblemSubmissionsPage() {
  ...
  const fetchSubmissions = useCallback(() => {
    ...
    apiFetch<{ submissions: Submission[] }>(`/api/submissions?problemId=${problemId}`)
      .then((data) => setSubmissions(data.submissions))
      .catch((err) => setError(err instanceof Error ? err.message : '제출을 불러오지 못했습니다.'));
  }, [problemId]);
  ...
  const hasRunning = useMemo(
    () => submissions.some((submission) => ['PENDING', 'RUNNING'].includes(submission.status)),
    [submissions]
  );
          ...
          <Table>
            ...
            <TableBody>
              ...
              {submissions.length === 0 && (
                <TableRow>
                  <TableCell colSpan={6}>
                    <Typography color="text.secondary">아직 제출 기록이 없습니다.</Typography>
                  </TableCell>
                </TableRow>
              )}
            </TableBody>

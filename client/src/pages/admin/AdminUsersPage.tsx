      ...
      <Card sx={{ borderRadius: 2, boxShadow: '0 16px 40px rgba(16,24,40,0.08)' }}>
        <CardContent>
          <Stack spacing={2}>
            ...
            <FormControl>
              <InputLabel id="user-select-label">대상 계정</InputLabel>
              <Select
                labelId="user-select-label"
                label="대상 계정"
                value={passwordUserId}
                onChange={(e) => setPasswordUserId(e.target.value === '' ? '' : Number(e.target.value))}
                disabled={isReadOnly}
              >
            ...
            <TextField
              ...
              disabled={isReadOnly}
            />
            <Button variant="contained" onClick={handlePasswordChange} disabled={isReadOnly}>
              변경
            </Button>
          </Stack>
        </CardContent>

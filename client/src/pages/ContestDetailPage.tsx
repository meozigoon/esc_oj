...
export default function ContestDetailPage() {
  ...
  useEffect(() => {
    ...
    apiFetch<{ contest: Contest }>(`/api/contests/${contestId}`)
      ...
      .catch((err) => setError(err instanceof Error ? err.message : '대회를 불러오지 못했습니다.'));

    apiFetch<{ problems: ProblemSummary[] }>(`/api/contests/${contestId}/problems`)
  ...
  if (!contest) {
    return (
      <Box>
        {error ? <Typography color="error">{error}</Typography> : <Typography>Loading...</Typography>}
      </Box>
    );
  }
          ...
          <List disablePadding>
            {problems.map((problem, index) => (
              <ListItem
                ...
                secondaryAction={
                  <Button component={Link} to={`/problems/${problem.id}`} variant="outlined">
                    문제 보기
                  </Button>
                }
              >
                <ListItemText
                  ...
                  secondary={`시간 제한 ${problem.timeLimitMs ?? '-'} ms | 메모리 제한 ${problem.memoryLimitMb ?? '-'} MB`}
                />
              </ListItem>
            ))}
            {problems.length === 0 && (

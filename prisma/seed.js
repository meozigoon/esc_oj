...
async function main() {
  ...
  const existingContest = await prisma.contest.findFirst({ where: { title: 'Sample Contest' } });
  ...
  const statementPath = toPosixPath('problems', String(problem.id), 'statement.mdx');
  ...
  for (const testcase of testcaseRows) {
    const inputPath = toPosixPath('problems', String(problem.id), 'tests', `${testcase.ord}.in`);
    const outputPath = toPosixPath('problems', String(problem.id), 'tests', `${testcase.ord}.out`);
